...
We provide the complete proof of Theorem~\ref{thm:unified_bound}. The main text states the bound at the operational alignment $W = I$ (trace form $\Omega$, Eq.~\ref{eq:omega_def}); here we establish it for the more general Procrustes-optimal alignment $W = W_N$ (nuclear form $\Omega_N$, Eq.~\ref{eq:omega_nuclear}), from which the $W = I$ specialization in the main text follows by direct expansion (the SVD step collapses since no maximization is needed). The proof proceeds through five self-contained steps: risk decomposition via triangle inequality, Lipschitz analysis of cross-entropy with respect to features, exact Procrustes decomposition via SVD, second-moment-adjusted head bound, and final assembly. The $W = W_N$ form underlies the Sec.~\ref{subsec:ablation} ablation; the $W = I$ form underlies all main-text experiments and the regularizer of Sec.~\ref{subsec:shape_reg}.
