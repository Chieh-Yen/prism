...
\paragraph{Setup.}
Recall that the risk of model $M$ is $\mathcal{R}_M = \mathbb{E}_{(x,y)}[\ell(\phi_M(x) \cdot H_M, y)]$, where $\ell$ is the cross-entropy loss (Eq.~\ref{eq:ce_def}). The proof below uses the Procrustes-optimal $W_N$; for any other $W \in \mathcal{O}(d)$ the same chain of inequalities holds with $W$ in place of $W_N$ throughout (only Step~3's SVD step specializes to $W_N$). Given $W_N \in \mathcal{O}(d)$ that maps proxy features toward target features (i.e., $\phi_T(x) \approx \phi_P(x) W_N$), we introduce an intermediate \emph{hybrid risk}:
\begin{equation}
\mathcal{R}_{P \to T} := \mathbb{E}_{(x,y)}\!\Big[\ell\!\big(\phi_P(x)\, W_N \cdot H_T,\; y\big)\Big].
\end{equation}
...
Using the Lipschitz property with $z_1 = \phi_T(x)$ and $z_2 = \phi_P(x) W_N$:
\begin{equation}
\begin{aligned}
\delta &= \big|\mathcal{R}_T - \mathcal{R}_{P \to T}\big| = \Big|\mathbb{E}\big[g_T(\phi_T(x), y) - g_T(\phi_P(x) W_N, y)\big]\Big| \\
&\le \mathbb{E}\Big[|g_T(\phi_T(x), y) - g_T(\phi_P(x) W_N, y)|\Big] \\
&\le K_{\mathrm{feat}} \cdot \mathbb{E}_x\!\big[\|\phi_T(x) - \phi_P(x) W_N\|_2\big].
\end{aligned}
\end{equation}
...
\delta \le K_{\mathrm{feat}} \cdot \sqrt{\mathbb{E}_x\!\big[\|\phi_T(x) - \phi_P(x) W_N\|_2^2\big]} = K_{\mathrm{feat}} \cdot \sqrt{\frac{1}{n}\|Z_T - Z_P W_N\|_F^2}.
\end{equation}
...
We seek the orthogonal matrix $W_N$ that minimizes the alignment residual:
\begin{equation}
W_N = \arg\min_{W \in \mathcal{O}(d)} \|Z_T - Z_P W\|_F^2.
\end{equation}
...
W_N = \arg\max_{W \in \mathcal{O}(d)} \operatorname{Tr}(Z_T^\top Z_P W).
\end{equation}
...
\end{equation}
Since $\sigma_i \ge 0$ and $|R_{ii}| \le 1$ for any orthogonal $R$, this is maximized when $R_{ii} = 1$ for all $i$, i.e., $R = I_d$, which yields $V^\top W_N U = I_d$ and thus:
\begin{equation}
\label{eq:W_optimal}
W_N = V U^\top.
\end{equation}
...
\end{equation}
Recalling $\rho_M = \frac{1}{\sqrt{n}}\|Z_M\|_F$ (so $\frac{1}{n}\|Z_M\|_F^2 = \rho_M^2$) and the definition of $\Omega_N$ (Eq.~\ref{eq:omega_nuclear}):
\begin{equation}
\frac{1}{n}\|Z_T^\top Z_P\|_* = \frac{\|Z_T^\top Z_P\|_*}{\|Z_T\|_F\|Z_P\|_F} \cdot \frac{\|Z_T\|_F\|Z_P\|_F}{n} = \Omega_N \cdot \rho_T \rho_P.
\end{equation}
Therefore:
\begin{equation}
\min_{W \in \mathcal{O}(d)} \frac{1}{n}\|Z_T - Z_P W\|_F^2 = \rho_T^2 + \rho_P^2 - 2\,\Omega_N\,\rho_T\rho_P.
\end{equation}
Adding and subtracting $2\rho_T\rho_P$ completes the square:
\begin{equation}
= (\rho_T^2 - 2\rho_T\rho_P + \rho_P^2) + 2\rho_T\rho_P - 2\,\Omega_N\,\rho_T\rho_P = \underbrace{(\rho_T - \rho_P)^2}_{\text{Scale Mismatch}} + \underbrace{2\rho_T\rho_P(1 - \Omega_N)}_{\text{Shape Mismatch}}.
\end{equation}
This is the $W = W_N$ specialization of Proposition~\ref{prop:exact_decomposition}; the general identity for any $W \in \mathcal{O}(d)$ follows from the same expansion with $\Omega_W$ in place of $\Omega_N$ (no SVD needed), and setting $W = I$ recovers the main-text trace form $\Omega$. \qed

\paragraph{Assembling the Feature Bound.}
Combining Eq.~(\ref{eq:delta_lipschitz}) with the decomposition above:
\begin{equation}
\label{eq:delta_final}
\delta \le K_{\mathrm{feat}} \sqrt{(\rho_T - \rho_P)^2 + 2\rho_T\rho_P(1 - \Omega_N)}.
\end{equation}
...
The head discrepancy measures how much the predictions differ when two different ``effective heads'' ($W_N H_T$ vs.\ $H_P$) operate on the same proxy features. Let $z = \phi_P(x)$ denote the (unaligned) proxy feature. Then:
\begin{equation}
\gamma = \big|\mathcal{R}_{P \to T} - \mathcal{R}_P\big| = \Big|\mathbb{E}\!\big[\ell(z \cdot W_N H_T, y) - \ell(z \cdot H_P, y)\big]\Big|.
\end{equation}
...
Define the head misalignment matrix $\Delta H = W_N H_T - H_P \in \mathbb{R}^{d \times V}$. The logit difference for a single sample is $z \cdot \Delta H = \phi_P(x)(W_N H_T - H_P)$, and:
\begin{equation}
\begin{aligned}
\gamma &\le \mathbb{E}\!\big[|\ell(z \cdot W_N H_T, y) - \ell(z \cdot H_P, y)|\big] \\
...
&\le K_{\mathrm{feat}} \sqrt{(\rho_T - \rho_P)^2 + 2\rho_T\rho_P(1 - \Omega_N)} + K_{\mathrm{pred}} \cdot \|\Sigma_P^{1/2}(W_N H_T - H_P)\|_F.
\end{aligned}
\end{equation}
