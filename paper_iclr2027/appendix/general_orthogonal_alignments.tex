...
\Big].
\end{equation}
Unlike the standard Procrustes problem (which minimizes $\delta$ alone and admits the closed-form $W_N = VU^\top$), the addition of $\gamma(W)$ introduces a quadratic dependence on $W$ through the head term, related to the \emph{Weighted Orthogonal Procrustes Problem} (WOPP)~\citep{gower2004procrustes}.
