...
| **E15** reference draw ablation(**paired**) | **B**;8VrD-Q3 | ~15 h(實測,12 cells × ~79 min) | ✅ **定稿(12/12,3 sizes × 4 互斥 draws,λ=1.0)**:n=8/16/32 → 0.690±0.019 / 0.685±0.011 / **0.676±0.008**,12 run 全在 −15.2%~−21.1%(最多動 5.9 pts);**canary 0.6829 vs 論文樹 0.6813(Δ0.0016)**;sd 隨 n 單調下降;論文值是典型 draw(比 4-draw 平均還差 0.0049)。draft (4) 已替換 | 只換「哪 n 條」,domain 軸刻意不動;**「不需要 in-task 資料」已撤回**(WikiText 32 列只交付 19 條、2.2× tokens、無 prompt 邊界 → unpaired)。**附帶勘誤**:E3.md §8.2 的 −51% 是拿錯 no-reg anchor(1.4830 為 pre-grad-fix 舊值,論文輪是 **0.8434**)→ E3.md §8.4 |
