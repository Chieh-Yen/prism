...
def main():
    ...
    lines.append(
        ...
        r"uniformly better ($|r_s|{=}1$ = perfect variant ranking). The three "
        r"rows compose the bound cumulatively---shape, then $+$ scale, then "
        r"$+$ head---so the row-to-row difference cleanly isolates each "
        r"component's contribution to ranking signal. The shape-only "
        r"similarity $\Omega_I$ already gives mean $|r_s|{=}0.77$; adding the "
        r"scale term to form the feature alignment error "
        r"$\delta_I{=}K_{\mathrm{feat}}\sqrt{(\Delta\rho)^2{+}"
        r"2\rho_T\rho_P(1{-}\Omega_I)}$ lifts this to 0.82 and wins the "
        r"plurality of cells---the largest marginal contribution of any "
        r"component. Adding the covariance-weighted head term to form the "
        r"full bound $\mathcal{B}_I$ brings the aggregate mean slightly down "
        r"to 0.79: our suite mixes protocols that quantize \texttt{lm\_head} "
        r"(GGUF k-quant tiers, $\gamma{>}0$) with protocols that keep it "
        r"FP16 (GPTQ and BnB, $\gamma{\equiv}0$), and this heterogeneity "
        r"injects non-monotone variance into the $\gamma$ term. "
        r"$\mathcal{B}_I$ nevertheless remains the certified upper bound of "
        r"Theorem~\ref{thm:unified_bound} and is strictly required when "
        r"$\gamma{>}0$; it also has the smallest cross-lineage spread "
