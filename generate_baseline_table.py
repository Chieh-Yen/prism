...
def main():
    ...
    lines.append(
        r"\caption{\textbf{Component-wise ablation of the PRISM bound} against "
        r"the empirical cross-entropy risk gap $|\Delta\mathcal{R}|$, measured "
        r"by per-cell Spearman rank correlation across the "
        rf"$4{{\times}}5$ PTQ grid ({n_cells_with_data} cells); larger $|r_s|$ is "
        r"uniformly better ($|r_s|{=}1$ = perfect variant ranking). All three "
        r"rows use the same $W{=}I$ operational alignment (Sec.~\ref{subsec:interpretation}), "
        r"so differences isolate which \emph{component} of the bound carries "
        r"ranking signal. The shape-only similarity $\Omega_I$ already gives "
        r"mean $|r_s|{=}0.77$; adding the scale term to form the feature "
        r"alignment error $\delta_I{=}K_{\mathrm{feat}}\sqrt{(\Delta\rho)^2{+}"
        r"2\rho_T\rho_P(1{-}\Omega_I)}$ lifts this to 0.82 and wins the "
        r"plurality of cells. Adding the covariance-weighted head term to "
        r"form the full bound $\mathcal{B}_I$ brings the aggregate mean "
        r"slightly down to 0.79, a regime artefact of our suite mixing "
        r"protocols that quantize \texttt{lm\_head} (GGUF k-quant tiers, "
        r"$\gamma{>}0$) with protocols that keep it FP16 (GPTQ and BnB, "
        r"$\gamma{\equiv}0$), which injects non-monotone variance into the "
        r"$\gamma$ term; $\mathcal{B}_I$ remains the certified upper bound "
        r"of Theorem~\ref{thm:unified_bound} and is strictly required when "
        r"$\gamma{>}0$, while $\delta_I$ empirically gives the tightest "
        r"single-number ranker when $\gamma$ is heterogeneous across "
        r"variants. Per-lineage columns additionally show that "
        r"$\mathcal{B}_I$ has the smallest cross-lineage spread "
        r"(per-lineage std $\approx$ 0.04 vs.\ 0.05--0.06 for $\Omega_I$ and "
        r"$\delta_I$), consistent with the head term stabilising outlier "
        r"lineages. ``Wins'' counts cells where the metric's $|r_s|$ is the "
        r"largest (tied winners counted for every tied metric); bold = best "
        r"in column.}"
    )
